...
def mol2vec(mol,mask_type="acid"):
    node_f = get_atom_features(mol)
    node_f = torch.tensor(node_f, dtype=torch.float)
    edge_index = get_bond_pair(mol)
    edge_index=torch.tensor(edge_index, dtype=torch.long)
    #calculate mask based on smart patterns
    deprotonatable_oxygen_smarts = '[CX3](=O)[OX2H1]'
    matches = mol.GetSubstructMatches(Chem.MolFromSmarts(deprotonatable_oxygen_smarts))
    if mask_type == "acid":
        mask = [False] * mol.GetNumAtoms()
        for match in matches:
    
            deprotonatable_oxygen_idx = match[2]
            mask[deprotonatable_oxygen_idx] = True
        ...
        mask_tensor = torch.tensor([1 if pka else 0 for pka in mask], dtype=torch.float).unsqueeze(1)
    elif mask_type == "base":
        mask_tensor = torch.tensor([1 if atom.GetSymbol() == 'N' and atom.GetFormalCharge() == 0 else 0 for atom in mol.GetAtoms()], dtype=torch.float).unsqueeze(1)

    return node_f,edge_index,mask_tensor
...
def predict_pka(smiles, model,mask_type="acid"):
    ...
    # Assuming mol2vec_with_mask is your function that generates features, edge indices, and mask
    x, edge_index, mask = mol2vec(mol,mask_type=mask_type)

    
    # Wrap the data into a Data object
    data = Data(x=x, edge_index=edge_index, mask=mask)
    
    # Batch the single data object (as the model expects batches)
    ...
    # Apply mask and extract pKa values
